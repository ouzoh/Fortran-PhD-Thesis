...
CHARACTER(LEN=20)                        :: dummyA,dummyC
REAL                                     :: dummyI
...
INTEGER                                  :: at_at
REAL,ALLOCATABLE,DIMENSION(:,:)          :: neigh,E_GAVR_at_Type,E_GAVD_at_Type
...
!open summary.r, read and store data
 ...
 ALLOCATE(atom_ty(M))              ! atom_ty   
 ALLOCATE(R(Imax,M-1,M))           ! interatomic distance
 ALLOCATE(neigh(M-1,M))            ! nearest neigbour
 ...
 ALLOCATE(E_GAVR_at_Type(Imax,Imax))!Gav. Repulson Energy grouped by atom type
 ALLOCATE(E_GAVD_at_Type(Imax,Imax))!Gav. Dispersion Energy grouped by atom type


 ALLOCATE(E_GAV_ND(Imax))          ! Gav. Energy not dominant
 ALLOCATE(E_GAVR_ND(Imax))         ! Gav. Repulson Energy not dominant
 ALLOCATE(E_GAVD_ND(Imax))         ! Gav. Dispersion Energy not dominant
 ...
 !Possible adaptation to increase atomic type
 ! REWIND(202)
 ! !Add C.sp3 atomic type
 ! DO I=1,M
 !     READ(202,*) dummyA,dummyA,dummyA,dummyA,dummyA,dummyC
 !     WRITE(*,*) I, dummyC
 !     IF (dummyC.EQ.'C.3') THEN
 !         WRITE(*,*) dummyC,'is C.sp3'
 !         atom_ele(I)='Ct'
 !     ELSE
 !         !do nothing
 !     END IF
 ...
 !Store all Gavezzotti's parameter!
 ...
 !######################################################!
 !evaluate exp-6 atom atom energies of all conformations
 !######################################################!
 ...
 DO J=1,Imax
     ...
     E_GAV(J,I,K)=0
     WRITE(208,*) "Conformation",J
     WRITE(208,*) "atI / atK / Rik / NEIGHik/ EGAVDik"
     DO I=1,M-1 !SUM OVER ATOM 
         DO K=I+1,M !SUM OVER ATOM J
             ...
             IF (neigh(I,K).GE.3) THEN
                 ...
                 WRITE(208,*) atom_ele(I),atom_ele(K),'/',R(J,I,K),'/',neigh(I,K),'/',E_GAVD(J,I,K)
             ELSE
                 CYCLE
             END IF
         END DO
...
3007 FORMAT(100(F8.3,1X))
!3008 FORMAT(50(A2,A1,A2,A1,3X))
3008 FORMAT(50(1X,2A,1A,2A,1X))
3009 FORMAT(2(I5),A5) !Integers
3010 FORMAT(A5,100(F14.7))
3011 FORMAT(100(1X,2A,1A,2A))



 !Determine the dominant atom-type...atom-type interactions
 PRINT*,''
 PRINT*,'####################################################################'
...
! CALL SYSTEM ('rm temp.txt')
     ...
     DO I=1,M-1 !SUM OVER ATOM 
         DO K=I+1,M !SUM OVER ATOM J
             ...
             ELSE IF ((neigh(I,K).GE.3).AND.(expr2.EQV.ans1)) THEN
                 ...
                 !2. calculate the dispersion for interactions not being
                 !fitted -----> now been fitted
                 E_POTD2=-(C(I,K)/(R(J,I,K)**6))
                 E_GAVD2(J)=E_GAVD2(J)+E_POTD2
                 !!!!!!!
                 E_GAVD_at_Type(J,at_at)=E_GAVD_at_Type(J,at_at)+E_POTD2
 ...
 PRINT*,'####################################################################'
 PRINT*,''
 WRITE (*,3008,ADVANCE='NO')('',trim(atom_D_ele1(N)),'-',trim(atom_D_ele2(N)),'', N=1,Mmax)
 WRITE (*,3008,ADVANCE='NO')('',trim(atom_D_ele1(N)),'-',trim(atom_D_ele2(N)),'', N=1,Mmax)
 WRITE (*,'(1A,4A,4A)') '','EAb-','EGRes'

 DO J=1,Imax
     PRINT 3007,(E_GAVR_at_Type(J,at_at),at_at=1,Mmax),&
               &(E_GAVD_at_Type(J,at_at),at_at=1,Mmax),&
...
!&/(E_GAVR_at_Type(1:Imax,1:Mmax))
 ...
 DO N=1,Mmax
     ...
     DO I=1,NcombL
         ...
         & rescale'
         WRITE(*, 3008) (trim(atom_D_ele1(c_int(K))),'-',trim(atom_D_ele2(c_int(K))),',',K=1,N)
        ...
        !END DO
         ...
         !!Can only handle 50 atom types
         !!Change 50 to appropriate number
         WRITE(212,'(50I3)',ADVANCE='NO')   (c_int(K),K=1,N) !AtomType numeric label
         !!Modification made
         WRITE(212,'(50I4)') (c_int(K)+Mmax,K=1,N) !additional atomic type for dispersion contribution
         WRITE(212,*)   Mmax             !Maximum number of AtomTypes // Pmax in nag
         WRITE(212,3009) Imax,(N*2)+2,"'U'"  !The number of data, number of parameter, and unweighted
         DO L=1,Imax
                          !1.00 for the intercept#
                          !Then the cosine# of the most flexible torsion [to
                          !potentially account for the electronic effect]
                          !Followed by the regressor# (repulsion and dispersion contribution in
                          !selected subset)
                          !##testing cos(deg) and cos(2*deg)##!
                          !##Also test 1-(tanh([2*deg-180])
                          !COS(1*DtoR*deg(L))
                          !,1-(TANH(1*DtoR*(2*deg(L)-180)))**2,
             WRITE(212,3010) '1.00',COS(2*DtoR*deg(L)),(E_GAVR_at_Type(L,c_int(K)),K=1,N),   &
                                                      &(E_GAVD_at_Type(L,c_int(K)),K=1,N),   & 
                          !!Ab initio minus All other contribution not fitted 
                          &((E_J(L))-(E_GAVR_ND(L)+E_GAVD_ND(L)))-                  &
                          
                          !a.repulsion not fitted
                          &((SUM(E_GAVR_at_Type(L,1:Mmax)))-                        &
                          &(SUM(E_GAVR_at_Type(L,c_int(1:N)))))-                    &
                          
                          !b.dispersion not fitted
                          &((SUM(E_GAVD_at_Type(L,1:Mmax)))-                        &
                          &(SUM(E_GAVD_at_Type(L,c_int(1:N))))),                    &
                          
                          !!Contributions not fitted
                          &((E_GAVR_ND(L)+E_GAVD_ND(L))+                            &
                          &((SUM(E_GAVR_at_Type(L,1:Mmax)))-                        &
                          &(SUM(E_GAVR_at_Type(L,c_int(1:N)))))+                    &
                          &((SUM(E_GAVD_at_Type(L,1:Mmax)))-                        &
                          &(SUM(E_GAVD_at_Type(L,c_int(1:N)))))),                   &
         ...
         WRITE(214,' (A29,I3,A6,I3,A1)') "      PARAMETER        (MMAX=",(N*2)+3,",NMAX=",Imax,")"
         ...
         WRITE(214,"(A35,I3,A3)" ) "      READ (NIN,*) (Param(K,1),K=1,",N*2,")"
         WRITE(214,"(A36,I3,A3)" ) "      WRITE(NOUT,*) (Param(K,1),K=1,",N*2,")"
         ...
         WRITE(214,"(a)" ) "     +,E_J(I,1) "
         !Tested for v.3 (at-at with cosine) working
         !Not full rank error for maximum results
         !WRITE(214,"(a)" ) "               WRITE(NOUT,*) (X(I,J),J=1,M), Q(I,1),E_J(I,1) "
...
!         WRITE(214,"(a)" ) "     +(mats/(IDF))**0.5" !#5
         ...
         WRITE(214,"(a)" ) "             DO 82 K=1,IP-2"
         WRITE(214,"(a)" ) "                    BP(Param(K,1))=B(K+2)"
!         WRITE(214,"(a)" ) "              WRITE (NOUT,'(1X,F10.4)',ADVANCE='NO') BP(K)"
         WRITE(214,"(a)" ) "   82        CONTINUE"
         
         WRITE(214,"(a)" ) "             DO 81 I=1,PMAX*2"
         ...
         WRITE(214,"(a)" ) "             WRITE (216,'(1X,F8.2,1x,F10.4)',ADVANCE='NO') B(2),B(1)"
